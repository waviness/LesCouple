<template>
	<view class="bg-white pb-100">
		...
		<view v-if="searchType === 1" class="mt-4 mx-4">
			...
			<view v-if="detail.showPet && (detail.pet === 1 || detail.pet === 0)" class="mt-1 color-gray-2 font-15">
				宠物：{{ detail.pet === 1 ? '有' : detail.pet === 0 ? '无' : '' }}
			</view>
			<view v-if="detail.showSituation && detail.situation" class="mt-1 color-gray-2 font-15">
				出柜情况：{{ detail.situationName }}</view>
			<view v-if="detail.showChild && (detail.child === 1 || detail.child === 0)"
				class="mt-1 color-gray-2 font-15">
		...
		<view v-else class="px-4">
			<view class="mb-3">{{ detail.hasMatchmaker ? `红娘微信：${detail.makerWechat}` : `她的微信：${detail.wechat}` }}
			</view>
			<AppTitle :title="'基本信息'" />
			<u-checkbox-group v-model="checkeds" placement="column">
				...
				<view class="d-flex mt-2 ml-6 color-gray-2 font-16">
					房产：{{ detail.house === 1 ? '有' : detail.house === 0 ? '无' : '' }}</view>
		...
		<view class="p-4">
			...
			<view v-if="detail.toAgeRange.length" class="mt-1 color-gray-2 font-15">年龄范围：{{ detail.toAgeRange[0] }} ~
				{{ detail.toAgeRange[1] }}
			</view>
			<view v-if="detail.sameCity" class="mt-1 color-gray-2 font-15">恋爱距离：{{ detail.sameCityName }}</view>
			<view v-if="detail.toEducation.length" class="mt-1 color-gray-2 font-15">学历要求：{{ detail.toEducationText }}
			</view>
	...
	export default {
		...
		methods: {
			async getDetail(userId) {
				...
				this.detail.charactersText = this.detail.characters ? this.detail.characters.split(',').map(item => {
					return findNameInList(item, charactersOptions)
				}) : ''
				this.detail.hobbyText = this.detail.hobby ? this.detail.hobby.split(',').map(item => {
					return findNameInList(item, hobbyOptions)
				}) : ''
				this.detail.entertainmentText = this.detail.entertainment ? this.detail.entertainment.split(',').map(
					item => {
				...
				this.detail.toEducationText = this.detail.toEducation ? this.detail.toEducation.map(item => {
			...
			add() {
				const hasMatchmaker = uni.getStorageSync('userInfo')?.hasMatchmaker
				if (hasMatchmaker) { // 我有红娘
					this.modalContent = '请右下角关注她后, 请您的红娘匹配'
					this.confirmText = '确认'
					this.modalType = 1
					this.modalShow = true
				} else if (this.detail.hasMatchmaker) { // 我没有，她有红娘
			...
			copyWechat() {
				uni.setClipboardData({
					data: this.detail.hasMatchmaker ? this.detail.makerWechat : this.detail.wechat,
					success: () => {
						this.$toast('复制成功')
					}
				})
